...
__webpack_require__.d(__webpack_exports__, {
  "detectPackageManager": () => (/* binding */ detectPackageManager),
  "installPackage": () => (/* binding */ installPackage),
  "uninstallPackage": () => (/* binding */ uninstallPackage)
});
...
async function pathExists(path2, type) {
  try {
    const stat = await promises_.stat(path2);
...
function* lookup(cwd = external_node_process_.cwd()) {
  let directory = external_node_path_.resolve(cwd);
...
async function detect(options = {}) {
  ...
  if (typeof options.stopDir === "string") {
    const resolved = external_node_path_.resolve(options.stopDir);
      ...
      switch (strategy) {
        case "lockfile": {
          for (const lock of Object.keys(LOCKS)) {
            if (await pathExists(external_node_path_.join(directory, lock), "file")) {
              const name = LOCKS[lock];
              const result = await parsePackageJson(external_node_path_.join(directory, "package.json"), options);
              if (result)
                return result;
              else
        ...
        case "install-metadata": {
          for (const metadata of Object.keys(INSTALL_METADATA)) {
            ...
            if (await pathExists(external_node_path_.join(directory, metadata), fileOrDir)) {
  ...
  try {
    ...
    if (nameAndVer) {
      ...
      if (name === "yarn" && ver && Number.parseInt(ver) > 1) {
        ...
        agent = "pnpm@6";
        return { name, agent, version };
      } else if (constants_AGENTS.includes(name)) {
        agent = name;
        return { name, agent, version };
      } else {
...
var external_node_module_ = __webpack_require__(82033);
// EXTERNAL MODULE: external "node:child_process"
var external_node_child_process_ = __webpack_require__(17718);
// EXTERNAL MODULE: external "node:stream"
var external_node_stream_ = __webpack_require__(84492);
// EXTERNAL MODULE: external "node:readline"
var external_node_readline_ = __webpack_require__(51747);
...
var u = /* @__PURE__ */ (0,external_node_module_.createRequire)(import.meta.url);
//#endregion
//#region src/env.ts
const d = /^path$/i;
const f = {
...
;// CONCATENATED MODULE: ./node_modules/@antfu/install-pkg/dist/index.js
// src/detect.ts


async function detectPackageManager(cwd = external_node_process_.cwd()) {
...
async function installPackage(names, options = {}) {
  ...
  if (agent === "pnpm") {
    ...
    if ((0,external_node_fs_.existsSync)((0,external_node_path_.resolve)(options.cwd ?? external_node_process_.cwd(), "pnpm-workspace.yaml"))) {
      args.unshift("-w");
    }
  }
  return R(
...
async function uninstallPackage(names, options = {}) {
  ...
  if (agent === "pnpm" && (0,external_node_fs_.existsSync)((0,external_node_path_.resolve)(options.cwd ?? external_node_process_.cwd(), "pnpm-workspace.yaml")))
    args.unshift("-w");
  return R(
    agent,
    [
