...
When `datadog-ci-version` uses a floating release selector such as `v5`, this action passes the workflow `github.token` to the install step by default so GitHub release resolution can be authenticated. To avoid depending on latest-within-major resolution, pin an exact `datadog-ci` tag such as `v5.6.0`.
